...
namespace Kampus.WordSearcher
{
    public static class Letter
    {
        ...
        public static readonly char wrongLetter = '#';
        ...
        static Tuple<int, int>[] basicPoints =
        {
            Tuple.Create(0, 0),
            Tuple.Create(2, 1),
            Tuple.Create(5, 6)
...
//            allLetters = letters;
        ...
        public static char GetLetter(this List<List<bool>> listMap, int x, int y, int w, int h)
        {
            ...
            for (int i = 0; i < hCheck; i++)
                for (int j = 0; j < wCheck; j++)
                    possibleLetters = listMap[(rowCheck[i] + x)%w][(helpCheckpoints[i, j] + y) % h]
        ...
        static bool ClarifyLetter(IReadOnlyList<List<bool>> listMap, char letter, int x, int y, int w, int h)
        {
            ...
            for (int i = 0; i < AI.height; i++)
                for (int j = 0; j < AI.width; j++)
                    if (listMap[(i + x)%w][(j + y)%h] ^ parseLetter[i][j])
                        return false;
            return true;
        }
        ...
        {
            return basicPoints.Any(point => listMap[(point.Item1 + x) % w][(point.Item2 + y) % h]);
        }
